...
First, we show how sockets together with the package `stream` can be used to 
...
While sockets are only used to connect to data steam generating processes,
web services are more versatile and can also be used
to deploy data stream clustering processes as a service.
...
This requires manually setting up connections, The example
is useful to understand how sockets work for users interested in 
implementing their own components using other programming languages. 
A more convenient way
to do this using package `streamConnect` is described later in this paper.
Here is 
a short example of how to set up communication using sockets.
The code in this example is not executed since it cannot be run in a 
markdown document.
...
`streamConnect` provides publish `DSD_via_Socket()` and the class
`DSD_ReadSocket` to create a socked based connection. 
`DSD_via_Socket()` creates a socket broadcasting data using a socket and
`DSD_ReadSocket` creates a `DSD` object reading from that socket.

## Server: Publish Data
We create a DSD process sending data to port port.
```{r eval = FALSE}
...
Here we only show how to deploy a clusterer.
...
## Server: Deploy a Web Service
Deploy a clustering process listening for data on port port.
```{r eval = TRUE}
library(streamConnect)
